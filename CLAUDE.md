...
- **D2 종료 기준: 서로 다른 공고 10건 이상의 지표 분포.** 1건이 멀쩡해 보이는 것은 기준이 아니다.
- **`temperature=0`은 재현성을 보장하지 않는다** (D2에 실측으로 확인). `seed`도 넣지 않는다.
  최소 검출 가능 효과는 실측 9%p이고, 요구사항 11개라 지표 눈금 자체가 9%p다.
  **공고 1건으로는 프롬프트 개선을 검출할 수 없다.** 공고를 늘리는 것이 유일한 해법이다.
